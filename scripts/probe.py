...
if ki and ki["verdict"] == "OK":
    ...
    limit_val = plan.get("credit_limit_monthly")
    tier = TIERS.get(limit_val, f"unrecognised ({limit_val})")
    print(f"\nPLAN      credit_limit_monthly = {limit_val}  ->  {tier}")
...
# --- publish a readable report into the repo --------------------------------
# Workflow logs need admin rights to read through the API, which makes them a poor
...
def shape_lines(node, depth=0, out=None, limit=60):
    ...
    if isinstance(node, dict):
        for k, v in list(node.items())[:40]:
            if isinstance(v, (dict, list)):
                out.append(f"{pad}{k}:")
                shape_lines(v, depth + 1, out, limit)
            else:
                out.append(f"{pad}{k} = {repr(v)[:70]}")
    elif isinstance(node, list):
        out.append(f"{pad}[{len(node)} items]")
        if node:
            shape_lines(node[0], depth + 1, out, limit)
    return out

lines = [
    "# Endpoint access, measured",
    "",
    f"Generated by `scripts/probe.py` on {time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())}.",
...
lines += [
    "",
    f"Controls: {'passed' if controls_ok else 'FAILED - treat this table as unreliable'}. ",
...
for r in results:
    if r["verdict"] != "OK" or not r.get("body"):
        continue
    d = (r["body"] or {}).get("data")
    if d is None:
        continue
    shapes += [f"## {r['label']}", "", f"`GET {r['path']}`", "",
               f"Returned: {len(d)} records" if isinstance(d, list) else "Returned: object", "",
               "```", *shape_lines(d[0] if isinstance(d, list) and d else d), "```", ""]
(DOCS / "payload-shapes.md").write_text("\n".join(shapes))

print(f"Wrote docs/endpoint-access.md and docs/payload-shapes.md")
print("Keep the raw output. It is the 'visible evidence of a real API call' the submission asks for.")
